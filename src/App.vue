...
export default {
	...
	methods: {
		...
		onClickEdit() {
			if (this.isEditingMode === true) {
				localStorage.setItem('entriesArray', JSON.stringify(this.entriesArray));
			}
	...
	created() {
		this.entriesArray = JSON.parse(localStorage.getItem('entriesArray'));
		this.rotationAngle = Math.PI / 2 - this.radianPerSlice / 2;
	}
}
